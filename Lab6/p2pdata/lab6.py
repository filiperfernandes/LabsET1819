...
def output(filename, mode, missing_range, dic_ips, dic_connection, dic_bandwidth, dic_packet_size, dic_time, prob_p2p, prob_np2p):
    lines = []
...
def main():
    ...
    if label:
        ...
        for filename in files:
            ...
            output(files_u[count], "label", dic_ips, dic_connection, dic_bandwidth, dic_packet_size, dic_time, prob_p2p, prob_np2p)
            count += 1
    ...
    if error is not None:
        for filename in files:
            print(filename)
            dic_ips, dic_connection, dic_bandwidth, dic_packet_size, dic_time, dic_p2p, missing_range = index(filename, "error", float(error))
            prob_p2p, prob_np2p = get_prob(dic_p2p)
            output(filename, "error", missing_range, dic_ips, dic_connection, dic_bandwidth, dic_packet_size, dic_time, prob_p2p, prob_np2p)
